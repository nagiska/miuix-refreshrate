...
class OverclockService : Service() {
    ...
    private fun applyOverclock(authMode: String, w: Int, h: Int, hz: Int) {
        try {
            val modes = com.refreshrate.control.util.AutoOverclockManager.getSupportedModes(this)
            ...
            if (target != null) {
                val currentHz = com.refreshrate.control.util.AutoOverclockManager.getCurrentRefreshRate(this)
                val allModes = modes
                when (authMode) {
